...
- [`context/routing-map.md`](context/routing-map.md): command/skill route IDs, exact-file baselines, and explicit triggers for broader context discovery
...
| [`process/`](process/README.md) | Shared delivery workflows such as PR preparation and review handoff |
...
- [`architecture/adr/template.md`](architecture/adr/template.md) for decisions that satisfy all ADR gates

Treat them as reference scaffolding until they are adapted to the real repo. Use the concise ADR template only when all three ADR gates in `architecture/adr/README.md` are satisfied.
...
- Select the command or skill route ID in `docs/context/routing-map.md`, start from its exact files,
  expand only when a documented trigger applies, and record why.
...
- Use `/speckit.aisdlc.pr` to draft an evidence-backed PR handoff before requesting provider review or creating external artifacts.
