...
metadata:
  ...
  source: ".specify/templates/commands/clarify.md"
---
...
- When `docs/context/routing-map.md` exists, select `command:speckit.clarify`; its durable-doc baseline is intentionally empty, so expand only from a concrete open question.
- If the map or matching row is missing, report `route ID: compatibility-fallback`; the fallback adds no durable document beyond the authority baseline, so expand only from a concrete open question.
- Load `.specify/memory/constitution.md` and `<feature_directory>/spec.md`.
- Review `docs/context/index.yaml` when it exists without loading unrelated durable documents.
- Preserve established terminology, constraints, and reusable patterns when resolving ambiguity.
