...
- `new-feature-candidate` and `bug-candidate`: invoke `/speckit.specify` with a concise,
...
- `enhance-existing-item`: name the verified existing-work path, ID, or link and ask to refine that
  artifact with the intake; do not invoke `/speckit.specify`, which could create a duplicate
  spec.
