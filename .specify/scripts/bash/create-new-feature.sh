...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --feature-dir)
            ...
            FEATURE_DIR_INPUT="$next_arg"
            ;;
        --timestamp)
            USE_TIMESTAMP=true
            ;;
        --help|-h)
            echo "Usage: $0 [--json] [--dry-run] [--allow-existing-branch] [--short-name <name>] [--number N] [--timestamp] [--feature-dir <path>] <feature_description>"
            echo ""
            echo "Options:"
            ...
            echo "  --feature-dir <path> Use an explicit feature directory inside the repository"
...
if [ -n "$FEATURE_DIR_INPUT" ]; then
    if [[ "$FEATURE_DIR_INPUT" = /* ]]; then
        FEATURE_DIR="$FEATURE_DIR_INPUT"
    else
        FEATURE_DIR="$REPO_ROOT/$FEATURE_DIR_INPUT"
    fi
    FEATURE_DIR="$(realpath -m "$FEATURE_DIR")"
    case "$FEATURE_DIR" in
        "$REPO_ROOT"/*) ;;
        *) echo "Error: --feature-dir must resolve inside the repository" >&2; exit 1 ;;
    esac
...
if [ "$DRY_RUN" != true ]; then
    ...
    if [ ! -f "$SPEC_FILE" ]; then
        ...
        if [ -n "$TEMPLATE" ] && [ -f "$TEMPLATE" ]; then
            ...
            echo "[MISSING_TEMPLATE] Spec template not found; no spec file was created" >&2
            exit 1
        fi
    fi
