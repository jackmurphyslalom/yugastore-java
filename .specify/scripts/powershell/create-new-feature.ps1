...
if ($Help) {
    Write-Host "Usage: ./create-new-feature.ps1 [-Json] [-DryRun] [-AllowExistingBranch] [-ShortName <name>] [-Number N] [-Timestamp] [-FeatureDir <path>] <feature description>"
    Write-Host ""
    Write-Host "Options:"
    ...
    Write-Host "  -FeatureDir <path>  Use an explicit feature directory inside the repository"
...
if ($FeatureDir) {
    $candidate = if ([System.IO.Path]::IsPathRooted($FeatureDir)) {
        [System.IO.Path]::GetFullPath($FeatureDir)
    } else {
        [System.IO.Path]::GetFullPath((Join-Path $repoRoot $FeatureDir))
    }
    ...
    if (-not $candidate.StartsWith($rootPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Error: -FeatureDir must resolve inside the repository"
    }
    $featureDir = $candidate
    $branchName = Split-Path -Leaf $featureDir
}
...
if (-not $DryRun) {
    ...
    if (-not (Test-Path -PathType Leaf $specFile)) {
        ...
        if ($template -and (Test-Path $template)) {
            ...
            throw "[MISSING_TEMPLATE] Spec template not found; no spec file was created"
        }
    }
