...
def _usage(argv0: str) -> str:
    return (
        ...
        "[--short-name <name>] [--number N] [--timestamp] [--feature-dir <path>] <feature_description>"
    )
...
Options:
  ...
  --feature-dir <path> Use an explicit feature directory inside the repository
...
def _parse_args(argv: list[str], argv0: str) -> Args:
    ...
    while i < len(argv):
        ...
        if arg == "--json":
            ...
            allow_existing = True
        elif arg in {"--short-name", "--number", "--feature-dir"}:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.feature_dir:
        requested = Path(args.feature_dir)
        feature_dir = (requested if requested.is_absolute() else repo_root / requested).resolve()
        try:
            feature_dir.relative_to(repo_root.resolve())
        except ValueError:
            print("Error: --feature-dir must resolve inside the repository", file=sys.stderr)
            return 1
    ...
    if not args.dry_run:
        ...
        if not spec_file.is_file():
            ...
            if template is not None and template.is_file():
                ...
                print(
                    "[MISSING_TEMPLATE] Spec template not found; no spec file was created",
                    file=sys.stderr,
                )
                return 1

        # Persist to .specify/feature.json so downstream commands can find the feature.
        persist_feature_json(repo_root, str(feature_dir.relative_to(repo_root)))
