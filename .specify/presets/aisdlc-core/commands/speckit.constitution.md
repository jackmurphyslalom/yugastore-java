...
- When `docs/context/routing-map.md` exists, select `command:speckit.constitution` and read only that row before triggered expansion.
- If the map or matching row is missing, report `route ID: compatibility-fallback` and read only `docs/product/overview.md`, `docs/product/glossary.md`, and `docs/architecture/overview.md` when they exist.
- Review `docs/context/index.yaml` and `docs/context/gaps.md` when they exist.
- If the durable context scaffold does not exist yet, inspect current repo evidence directly before writing new principles.
- Review the existing constitution plus route-selected context to avoid conflicting guidance.
- Capture only durable, project-wide rules and terminology. Keep feature-specific details in specs and plans instead.
