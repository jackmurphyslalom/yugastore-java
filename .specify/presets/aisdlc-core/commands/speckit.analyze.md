...
- When `docs/context/routing-map.md` exists, select `command:speckit.analyze` and read only that row before triggered expansion.
- If the map or matching row is missing, report `route ID: compatibility-fallback` and read only `docs/product/overview.md`, `docs/architecture/overview.md`, and `docs/patterns/README.md` when they exist.
- Load `.specify/memory/constitution.md`, `<feature_directory>/spec.md`, `<feature_directory>/plan.md`, and `<feature_directory>/tasks.md`.
- Review `docs/context/index.yaml` when it exists and record why each route-selected or triggered file applies.
- Ground findings in repo-specific terminology, patterns, and constraints instead of generic best-practice defaults.
