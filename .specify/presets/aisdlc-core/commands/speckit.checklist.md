...
- When `docs/context/routing-map.md` exists, select `command:speckit.checklist` and read only that row before triggered expansion.
- If the map or matching row is missing, report `route ID: compatibility-fallback` and read only `docs/product/overview.md` and `docs/patterns/README.md` when they exist.
- Load `.specify/memory/constitution.md`, relevant feature artifacts, and `docs/context/index.yaml` when it exists.
- Reuse repo-specific terminology, standards, and patterns so the checklist reflects the real codebase.
