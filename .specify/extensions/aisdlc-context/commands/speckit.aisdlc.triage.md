...
- explicitly recommend `__SPECKIT_COMMAND_AISDLC_BOOTSTRAP__` as a follow-up when better durable context would materially improve future triage
...
- `new-feature-candidate`: the intake describes desired new behavior or scope and no credible existing spec, issue, user story, or backlog item was found that it extends
...
- `enhance-existing-item`: the current direction appears intentional, the intake adds scope, evidence, acceptance detail, or a usability gap, and you identified a specific existing spec, issue, user story, or backlog item that the intake logically extends
...
An `enhance-existing-item` recommendation requires a concrete existing-work reference in both
`Related Work Or Overlap` and `Primary Recommendation`: include its path, ID, or link and explain why
the intake extends it. If no credible existing item was found, do not use `enhance-existing-item`; use
`new-feature-candidate` for otherwise clear new product work, or `needs-clarification` when the intake
...
- open or refine a spec with `__SPECKIT_COMMAND_SPECIFY__`
- review backlog or roadmap priority manually
- link the intake as evidence to an existing item
- run `__SPECKIT_COMMAND_AISDLC_PROMOTE__` if durable context appears stale or incomplete
- gather clarification before routing the intake further

### Copy/Paste Next-Step Prompt

Provide exactly one fenced `text` block containing one ready-to-run or ready-to-send next-step prompt.
Use the primary recommendation to choose it:

- `new-feature-candidate` and `bug-candidate`: invoke `__SPECKIT_COMMAND_SPECIFY__` with a concise,
  normalized request derived from the intake.
- `enhance-existing-item`: name the verified existing-work path, ID, or link and ask to refine that
  artifact with the intake; do not invoke `__SPECKIT_COMMAND_SPECIFY__`, which could create a duplicate
  spec.
- `already-covered`: ask the user to review the identified existing item and attach or link the intake
  as evidence if appropriate.
- `needs-clarification`: ask the smallest concrete question needed to route the intake.
- `no-action`: state the recommended disposition and that no new spec or backlog item should be created.

The prompt must preserve the evidence or existing-work reference that justified the recommendation.
